...
def _control_stream_loop(snaking_instance: 'Snaking'):
    try:
        stream = snaking_instance._stub.ControlChannel(snaking_instance._handshake())
        snaking_instance._control_stream = stream
        
        for cmd in stream:
            if cmd.command == pb.OrchestratorCommand.PAUSE:
                logger.info(f'Worker {snaking_instance._id} received PAUSE command.')
            elif cmd.command == pb.OrchestratorCommand.STOP:
                logger.info(f'Worker {snaking_instance._id} received STOP command.')
                snaking_instance.status = WorkerStatus.STOPPED
                break
    except grpc.RpcError as e:
        if e.code() in (grpc.StatusCode.UNKNOWN, grpc.StatusCode.CANCELLED, grpc.StatusCode.UNAVAILABLE):
            logger.debug(f'Control stream closed: {str(e)}')
        else:
            logger.error(f'Control stream error in worker {snaking_instance._id}: {str(e)}')
        snaking_instance.status = WorkerStatus.STOPPED
    except Exception as e:
        logger.error(f'Unexpected error in control stream of worker {snaking_instance._id}: {str(e)}')
        snaking_instance.status = WorkerStatus.STOPPED
...
class Snaking:
    ...
    def keep_on(self) -> bool:
        with self._lock:
            if self.final_step > 0 and self.current_step >= self.final_step:
    ...
    def _handshake(self, heartbeat_interval: float = 3.0):
        self._status = WorkerStatus.RUNNING.value
        yield pb.WorkerStatus(worker_id=self._id, status=self._status, message='Connected')
        
        while self.keep_on:
            try:
                time.sleep(heartbeat_interval)
                yield pb.WorkerStatus(worker_id=self._id, status=self._status, message='Heartbeat')
            except Exception:
                self._status = WorkerStatus.STOPPED.value
                yield pb.WorkerStatus(worker_id=self._id, status=self._status, message='Error')
                break
        ...
        while True:
            ...
            try:
                ...
                if res.success:
                    ...
                    stream_thread = threading.Thread(target=_control_stream_loop, args=(self,), daemon=True)
                    stream_thread.start()
                    return
                else:
    ...
    def _cleanup(self):
        self.status = WorkerStatus.STOPPED
        if self._stub.ControlChannel is not None:
